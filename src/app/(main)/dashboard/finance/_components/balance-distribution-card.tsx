...
const balanceData = [
  { account: "Main Wallet", amount: 122_540, key: "main", percentage: 52.2 },
  { account: "Savings Account", amount: 48_320, key: "savings", percentage: 20.6 },
  { account: "Investment Account", amount: 36_780, key: "investment", percentage: 15.7 },
  { account: "Reserve Account", amount: 27_256, key: "reserve", percentage: 11.5 },
] as const satisfies readonly { account: string; amount: number; key: BalanceKey; percentage: number }[];
...
const currencies = {
  EUR: { label: "Euro Balance" },
  GBP: { label: "GBP Balance" },
...
export function BalanceDistributionCard() {
  ...
  const chartData = balanceData.map((item) => ({
    ...item,
    fill: "color" in chartConfig[item.key] ? chartConfig[item.key].color : undefined,
  }));
