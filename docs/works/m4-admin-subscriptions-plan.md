...
| TASK-010 | Extend `src/modules/assets/{types,schemas,repositories,services}.ts` only where necessary to support M4 quick-add reuse or candidate-asset metadata reuse. Keep all asset creation canonical in the asset domain. Do not create a second asset-write path in the subscriptions module. |  |  |
