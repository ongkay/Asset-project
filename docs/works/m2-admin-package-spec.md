---
title: Milestone 2 Admin Package Implementation Specification
version: 1.0
date_created: 2026-04-14
last_updated: 2026-04-14
owner: AssetProject
tags: [process, admin, package, nextjs, insforge, milestone-2]
---

# Introduction
This specification defines the implementation contract for Milestone 2 Admin Package. It is written for AI coding agents and maintainers that must implement the package-management milestone without inventing behavior that conflicts with `docs/PRD.md`, `docs/DB.md`, `docs/IMPLEMENTATION_PLAN.md`, and the baseline SQL migrations.

Milestone 2 is complete only when an admin can create, edit, search, filter, paginate, and enable or disable packages from a real browser route, with entitlement validation based on exact `platform + asset_type` access keys and a server-side read model for `total used`.

## 1. Purpose & Scope

### 1.1 Purpose
The purpose of this specification is to provide a machine-readable contract for the Milestone 2 Admin Package feature so implementation can be executed consistently across UI, server actions, read models, and database-backed validation.
...
### 1.3 Out of Scope
- Subscription activation flows in Milestone 4, Milestone 5, and Milestone 6.
...
- **REQ-003**: Milestone scope and definition of done must follow Milestone 2 in `docs/IMPLEMENTATION_PLAN.md`.
...
- **CON-007**: Do not add a separate browser test file only to satisfy the milestone gate.
...
- **AC-012**: Given a package is edited or disabled, when later milestones inspect historical subscriptions or issued CD-Keys, then existing history remains valid and unchanged.
...
## 7. Rationale & Context
The package entity is an upstream contract for later milestones. Subscription activation, CD-Key issuance, and payment-dummy activation all depend on package snapshots, so package data must be stable, validated, and reusable without introducing hidden behavior.

`access_keys_json` is the authoritative entitlement source because the database schema validates it directly and later milestones snapshot the same entitlement list into subscriptions and CD-Keys. The package summary is intentionally derived only for UI and reporting so it cannot drift into being an authorization source.

Disable and enable are preferred over hard delete because package history must remain intact for subscriptions and CD-Keys that were created earlier. This preserves future milestone compatibility and avoids historical data loss.
