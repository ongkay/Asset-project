...
- login atau register yang berhasil wajib:
...
- tombol `Reset Password` di flow login baru tampil setelah 5 gagal login berturut-turut untuk email yang sama
...
- `/reset-password` state set password baru setelah token valid
...
| ------- | ------------------------------------------------------------ | ---------------------------------------- | ------------------------------------------------------------------------------------------------------------- |
| 1       | Form dengan field `Email` dan tombol `Next`                  | Input email lalu klik `Next`             | Validasi format email lalu cek ke server apakah email sudah terdaftar                                         |
...
| 4A      | Error login jika password salah                              | Coba lagi dengan password yang benar     | Failed counter untuk email itu bertambah                                                                      |
...
| 5       | Tidak ada layar sukses yang panjang; flow langsung berlanjut | Login berhasil                           | Sistem revoke session lama, membuat session baru, menyimpan cookie `app_session`, lalu redirect ke `/console` |
...
| 3       | Halaman atau step lanjutan setelah token reset valid             | Membuka link / token reset yang diterima | Sistem memvalidasi token reset                                                                            |
| 4       | Form set password baru                                           | Input password baru lalu submit          | Sistem menyimpan password baru jika token valid                                                           |
...
- redirect ke `/console` baru dilakukan setelah session baru benar-benar siap dipakai
...
- flow sukses login dan register sebaiknya terasa cepat karena akhir flow yang benar adalah redirect ke `/console`, bukan berhenti di halaman auth
...
- form set password baru setelah token valid
- password input dengan show/hide
